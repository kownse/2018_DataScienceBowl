...
def conv_block(input_tensor, kernel_size, filters, stage, block,
               strides=(2, 2), use_bias=True, se=False):
...
def resnet_graph(input_image, architecture, stage5=False):
    assert architecture in ["resnet50", "resnet101", "se_resnet50"]

    se = 'se_' in architecture
    #print('\nse = {}\n'.format(se))
    # Stage 1
    ...
    # Stage 2
    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), se=se)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='b', se=se)
    C2 = x = identity_block(x, 3, [64, 64, 256], stage=2, block='c', se=se)
    # Stage 3
    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', se=se)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b', se=se)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c', se=se)
    C3 = x = identity_block(x, 3, [128, 128, 512], stage=3, block='d', se=se)
    # Stage 4
    x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a', se=se)
    block_count = {"resnet50": 5, "resnet101": 22, "se_resnet50": 5}[architecture]
    for i in range(block_count):
        x = identity_block(x, 3, [256, 256, 1024], stage=4, block=chr(98 + i), se=se)
    C4 = x
    # Stage 5
    if stage5:
        x = conv_block(x, 3, [512, 512, 2048], stage=5, block='a', se=se)
        x = identity_block(x, 3, [512, 512, 2048], stage=5, block='b', se=se)
        C5 = x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c', se=se)
    else:
        C5 = None
...
def load_image_gt(dataset, config, image_id, augment=False,
                  use_mini_mask=False, augparam = -1):
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
                   batch_size=1, detection_targets=False, augparam = -1):
        ...
        try:
            ...
            image, image_meta, gt_class_ids, gt_boxes, gt_masks = \
                load_image_gt(dataset, config, image_id, augment=augment,
                              use_mini_mask=config.USE_MINI_MASK, augparam = augparam)
...
class MaskRCNN():
    ...
    def find_last(self, searchdir = None):
        ...
        if searchdir is None:
            searchdir = self.model_dir
        dir_names = next(os.walk(searchdir))[1]
        ...
        if not dir_names:
            return None, None
        # Pick last directory
        dir_name = os.path.join(searchdir, dir_names[-1])
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, argument = True):
        """Train the model.
        ...
        #valid_batch_size = min(len(val_dataset.image_ids), self.config.BATCH_SIZE)
        #print('\nValid BatchSize = {}\n'.format(valid_batch_size))
        
        val_generator = None
        if val_dataset:
            val_generator = data_generator(val_dataset, self.config, shuffle=True,
                                       batch_size= len(val_dataset.image_ids),
                                       augment=False)
        ...
        def stackList(X_stacked, X_new):
            for i in range(len(X_stacked)):
                X_stacked[i] = np.vstack((X_stacked[i], X_new[i]))
            return X_stacked
        ...
        original = data_generator(train_dataset, self.config, shuffle=True, augment=False,
                                        batch_size = len(train_dataset.image_ids))
        X_stacked, Y_stacked = next(original)
        
        if argument:
            print('generate fix rotate')
            for i in tqdm(range(3)):
                train_generator = data_generator(train_dataset, self.config, shuffle=True,
                                            batch_size = len(train_dataset.image_ids),
                                            augment=True, augparam=i)

                x, y = next(train_generator)
                X_stacked = stackList(X_stacked, x)
                Y_stacked = stackList(Y_stacked, y)
                
            """
            rotate_generator = data_generator(train_dataset, self.config, shuffle=True,
                                            batch_size = len(train_dataset.image_ids),
                                            augment=True, augparam=3)
            
            print('generate random rotate')
            for i in tqdm(range(self.config.STEPS_PER_EPOCH)):
                x, y = next(rotate_generator)
                X_stacked = stackList(X_stacked, x)
                Y_stacked = stackList(Y_stacked, y)
            """
            
        #train_batchsize = min(len(train_dataset.image_ids), self.config.BATCH_SIZE)
        #print('\nTrain BatchSize = {}\n'.format(train_batchsize))
        
        validation_data = next(val_generator) if val_generator is not None else None

        if len(X_stacked[0].shape) < 4:
                X_stacked[0] = X_stacked[0][:,:,:, np.newaxis]
        if len(validation_data[0][0].shape) < 4:
                validation_data[0][0] = validation_data[0][0][:,:,:, np.newaxis]
        self.keras_model.fit(X_stacked, Y_stacked,
